...
import bandesal.gob.sv.cif.anexos.form.SolicitudReqModificaAnexosForm;
...
public class SolicitudReqModificaAnexosAction extends Action{
    private final static String SUCCESS = "successModifica";
    final static Logger logger = Logger.getLogger(SolicitudReqModificaAnexosAction.class);

    public ActionForward execute(ActionMapping mapping, ActionForm form,
            HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        SolicitudReqModificaAnexosForm anexos = (SolicitudReqModificaAnexosForm) form;
                ...
                if ("BORRAR".equals(opcion)) {
                    if(isTokenValid(request)) {
                        ...
                        if (StringUtils.isNotBlank(mensaje)) {
                            throw new Exception("Problemas en modifica anexos al eliminar solicitud de requerimiento");
                        } else {
                            idBir = Util.guardarBitacoraRequerimiento(idReq, usuario, estado );
                        }
                    } else {
                        logger.info("invalid token modificar anexo borrar solicitud");
                    }
